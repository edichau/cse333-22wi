...
//  Edited By Travis Quincy McGaha May 2021 since me.com links are dead
//  Copyright (c) 2007-2008 Apple Inc. All rights reserved.
//
////////////////////////////////////////////////////////////////////////////////

var slideWidth;var slideHeight;var frameHeight;var frameWidth;var scroller;var slideshow;var thumbnails;var hud;var thumbMatte=89;var pickerHeight=100;var browser;var baseURL="./";var windowWidth=800;var windowHeight=800;function initSlideshow(imageStream,index,parameters){browser=new BrowserDetectLite();if(checkBrowser())
...
if(browser.supportsPNGs){backButton=appendImage(controls,baseURL+'canvas_arrow-left_01.png','backbutton');playButton=appendImage(controls,baseURL+'canvas_play_01.png','playbutton',null,null,true);pauseButton=appendImage(controls,baseURL+'pause.png','pausebutton');forwardButton=appendImage(controls,baseURL+'canvas_arrow-right_01.png','forwardbutton');}else{controls.style.background='url('+baseURL+'images/controls.gif) center center no-repeat';backButton=appendImage(controls,baseURL+'canvas_arrow-left-D_01.gif','backbutton');playButton=appendImage(controls,baseURL+'images/play.gif','playbutton',null,null,true);pauseButton=appendImage(controls,baseURL+'images/pause.gif','pausebutton');forwardButton=appendImage(controls,baseURL+'images/arrow_right.gif','forwardbutton');}
...
function initScroller(startPoint){var thumbZone=$('thumbnailPicker');var scrollbar=appendDiv(thumbZone,"scrollbar");scrollbar.style.top="84px";scrollbar.style.left=0;scrollbar.style.width=thumbZone.getWidth()-2+"px";scrollbar.style.height="15px";var bar=appendDiv(scrollbar,"bar");bar.style.top=0;bar.style.left=0;bar.style.width="100%";bar.style.height="13px";var dragTool=appendDiv(scrollbar,"dragtool");dragTool.style.top="1px";dragTool.style.left=0;dragTool.style.width="26px";dragTool.style.height="13px";dragTool=$(appendImage(dragTool,baseURL+'dragger.png',null,24,13));dragTool.setStyle({margin:'0 1px'});var ruler=appendDiv(scrollbar,"ruler");ruler.style.top=0;ruler.style.left=0;scroller=new Scroller();if(startPoint!=null){scroller.jumpTo(startPoint);}
...
function stop(){slideshow.pause();slideshow.playHandler=-1;$('playbutton').style.display='';$('pausebutton').src=browser.supportsPNGs?baseURL+'pause.png':baseURL+'images/pause.gif';$('pausebutton').style.display='none';return false;}
function pauseClick(){$('pausebutton').src=browser.supportsPNGs?baseURL+'pause.png':baseURL+'transparent.gif';return false;}
function restart(){slideshow.playHandler=null;$('pausebutton').style.display='';$('playbutton').src=browser.supportsPNGs?baseURL+'canvas_play_01.png':baseURL+'transparent.gif';$('playbutton').style.display='none';if($('controls').getOpacity()>0.05)
{hud.fade(0.90,0,5000);}
slideshow.playHandler=setTimeout(function(){slideshow.resume();},1500);}
function playClick(){$('playbutton').src=browser.supportsPNGs?baseURL+'canvas_play_01.png':baseURL+'transparent.gif';return false;}
function forwardClick(){$('forwardbutton').src=browser.supportsPNGs?baseURL+'canvas_arrow-right_01.png':baseURL+'images/arrow_right_on.gif';return false;}
function next(){$('forwardbutton').src=browser.supportsPNGs?baseURL+'canvas_arrow-right_01.png':baseURL+'images/arrow_right.gif';slideshow.advance();return false;}
function backClick(){$('backbutton').src=browser.supportsPNGs?baseURL+'canvas_arrow-left_01.png':baseURL+'images/arrow_left_on.gif';return false;}
function previous(){$('backbutton').src=browser.supportsPNGs?baseURL+'canvas_arrow-left_01.png':baseURL+'images/arrow_left.gif';slideshow.goBack();return false;}
